...
public class UlpayRaTools {
    ...
    private static String EntCertPath;
    private static String CertPass;
    private static String CvmPath;
    private static String mercCertPath; //商户证书路径
    private static String mercCertPass;  //商户证书密钥
    private static String mercCvmPath;
    private static String chooseCert;
    private static String mercGmCertPath;
    private static String mercGmCertPass;
    private static String mercGmTrustPath;
    ...
    private UlpayRaTools() {
        ...
        try {
            ...
            InputStream in = new BufferedInputStream(new FileInputStream(basePath + "config.properties"));
            prop.load(in);     ///加载属性列表
        } catch (Exception e) {
            e.printStackTrace();
        }
        EntCertPath = basePath + prop.get("EntCertPath");
        CertPass = prop.get("CertPass") + "";
        CvmPath = basePath + prop.get("CvmPath");
        mercCertPath = basePath + prop.get("mercCertPath");
        mercCertPass = prop.get("mercCertPass") + "";
        mercCvmPath = basePath + prop.get("mercCvmPath");
        chooseCert = prop.get("chooseCert") + "";
        mercGmCertPath = basePath + prop.get("mercGmCertPath");
        mercGmCertPath = mercGmCertPath.substring(1);
        mercGmCertPass = prop.get("mercGmCertPass") + "";
        mercGmTrustPath = basePath + prop.get("mercGmTrustPath");
     ...
     * @Description: 商户加签
     */
    public String mercSign(String oriData) {
        String signData = "";
        if (chooseCert.equals("CFCA")) {
