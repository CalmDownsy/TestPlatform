...
public class TestAgreementPayGetpayurl extends RequestMsgUtils{
    ...
    public void getGetPayUrlInit() throws Exception{
        ...
        bodyMap.put("ORDER_EXP_DATE","20970724101011");//订单过期时间
...
//		TRANS_DETAILS.add(TRANS_DETAIL1);
    ...
    public void testGetPayUrl_Ordinary() throws Exception {
    	logger.info("###  开始执行 获取签约并支付URL(200014)（merc-front-web/agreementpay/getpayurl） 用例...");
    	getGetPayUrlInit();	
        ...
        logger.info("###  开始执行 签约并支付发送短信（merc-front-web/agreementpay/paysms） 用例...");
    	/*签约发送短信请求参数*/
//        signPaySmsInit();
        ...
        logger.info("###  开始执行 绑卡并支付（merc-front-web/agreementpay/bindcardpay） 用例...");
        /*绑卡签约请求参数*/
//        bindCardPayInit();
        System.out.println(platOrderNo);
        System.out.println(userNo);
